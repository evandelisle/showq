...
Properties::Properties(BaseObjectType *c_object, const Glib::RefPtr<Gtk::Builder> &refXml)
    : Gtk::Dialog(c_object),
      m_refXml(refXml)
{
...
{
    Properties *dialog;
    auto refXml = Gtk::Builder::create_from_resource("/org/evandel/showq/ui/prop.ui");
    refXml->get_widget_derived("Properties", dialog);

    return std::unique_ptr<Properties>(dialog);
}

Preferences::Preferences(BaseObjectType *c_object, const Glib::RefPtr<Gtk::Builder> &refXml)
    : Gtk::Dialog(c_object),
      m_refXml(refXml)
{
...
{
    Preferences *dialog;
    auto refXml = Gtk::Builder::create_from_resource("/org/evandel/showq/ui/pref.ui");
