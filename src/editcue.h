...
public:
    Fader(BaseObjectType *c_object, const Glib::RefPtr<Gtk::Builder> &refXml);
...
protected:
    Glib::ustring on_format_value(double v) override;
};
...
public:
    ...
    ~EditCueMidi() override;

    void get(std::shared_ptr<Cue> &q) override;
    void set(std::shared_ptr<Cue> &q) override;
...
public:
    EditCue(BaseObjectType *c_object, const Glib::RefPtr<Gtk::Builder> &refXml);
    ~EditCue() override;
