...
EditCueBase::EditCueBase() = default;

EditCueBase::~EditCueBase() = default;

EditCue::EditCue(BaseObjectType *c_object, const Glib::RefPtr<Gtk::Builder> &refXml)
    : Gtk::Window(c_object),
      m_refXml(refXml),
      cue_id_no(nullptr)
{
...
EditCue *EditCue::show(int type)
{
    ...
    EditCue *p = nullptr;
    auto refXml = Gtk::Builder::create_from_resource("/org/evandel/showq/ui/editcue.ui");
    ...
    switch (type) {
    case Cue::MIDI:
        p->m_tabs = std::make_shared<EditCueMidi>(p->m_notebook);
        break;
    case Cue::Wave:
        p->m_tabs = std::make_shared<EditCueWave>(p->m_notebook);
        break;
    case Cue::Stop:
        p->m_tabs = std::make_shared<EditCueStop>(p->m_notebook);
        break;
    case Cue::Fade:
        p->m_tabs = std::make_shared<EditCueFade>(p->m_notebook);
        break;
    case Cue::Group:
        p->m_tabs = std::make_shared<EditCueGroup>(p->m_notebook);
        break;
    case Cue::Pause:
        p->m_tabs = std::make_shared<EditCuePause>(p->m_notebook);
        break;
    case Cue::Start:
        p->m_tabs = std::make_shared<EditCueStart>(p->m_notebook);
        break;
    }
...
{
    const std::shared_ptr<Cue> tq = (*i)[app->m_refTreeModel->Col.cue];
...
{
    const std::shared_ptr<Cue> q = (*i)[app->m_refTreeModel->Col.cue];
...
bool EditCue::on_key_release_event(GdkEventKey *event)
{
    if (m_key_but->get_active()) {
        ...
        if (keyval == GDK_KEY_BackSpace)
            keyval = 0;
        m_key_but->set_label(keyval ? Gtk::AccelGroup::name(keyval, state) : _("Disabled"));
        return true;
    }
