...
Patch::Patch(BaseObjectType *c_object, const Glib::RefPtr<Gtk::Builder> &refXml)
    : Gtk::Dialog(c_object),
      m_refXml(refXml)
{
    try {
    ...
    for (int i = 1; i < 9; ++i) {
        const Gtk::TreeModel::iterator iter = m_refOports->append();
    ...
    m_oports->get_selection()->signal_changed().connect(sigc::mem_fun(*this, &Patch::refresh));
...
void Patch::row_change1(const Gtk::TreeModel::Path &, const Gtk::TreeModel::iterator &iter)
{
    ...
    const int port = row[m_Columns.m_col_port];
    const bool enable = row[m_Columns.m_col_enable];
    const Glib::ustring name = row[m_Columns.m_col_text];

    const int i = snd_seq_get_port_info(oseq, port, info);
...
void Patch::row_change2(const Gtk::TreeModel::Path &, const Gtk::TreeModel::iterator &iter)
{
    ...
    const Gtk::TreeModel::iterator j = m_oports->get_selection()->get_selected();
    if (!j || !(*j)[m_Columns.m_col_enable])
        return;

    const int midi_port = (*j)[m_Columns.m_col_port];
    const int client = (*iter)[m_Columns.m_col_client];
    const int port = (*iter)[m_Columns.m_col_port];
...
void Patch::refresh()
{
    ...
    while (snd_seq_query_next_client(oseq, cinfo) >= 0) {
        ...
        while (snd_seq_query_next_port(oseq, info) >= 0) {
            auto caps = snd_seq_port_info_get_capability(info);
            auto type = snd_seq_port_info_get_type(info);

            if (caps & SND_SEQ_PORT_CAP_NO_EXPORT)
                continue;
            if (!((caps & SND_SEQ_PORT_CAP_WRITE) || (caps & SND_SEQ_PORT_CAP_DUPLEX)))
                continue;
            if (type == SND_SEQ_PORT_SYSTEM_TIMER)
                continue;
            if (type == SND_SEQ_PORT_SYSTEM_ANNOUNCE)
                continue;

            const Gtk::TreeModel::iterator iter = m_refTreeModel->append();
            const Gtk::TreeModel::Row &row = *iter;
            row[m_Columns.m_col_text] = snd_seq_port_info_get_name(info);
...
{
    Patch *p;
    auto refXml = Gtk::Builder::create_from_resource("/org/evandel/showq/ui/patch.ui");
