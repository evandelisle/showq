...
template <class T> class RCUManager {
public:
    ...
    [[nodiscard]] std::shared_ptr<T> reader() const { return *m_rcu_value.load(); }
...
template <class T> class SerializedRCUManager : public RCUManager<T> {
public:
    ...
    bool update(std::shared_ptr<T> new_value) override
    {
        ...
        auto *new_spp = new std::shared_ptr<T>(new_value);
...
template <class T> class RCUWriter {
public:
